...
const handleEngineChange = async (train_id, newEngineId) => {
  ...
  try {
    ...
    if (train.current_engine && !train.current_engine.equals(newEngine._id)) {
      const currentEngine = await Engine.findById(train.current_engine);
      if (currentEngine) {
    ...
    train.current_engine = newEngine._id;
    await train.save();
