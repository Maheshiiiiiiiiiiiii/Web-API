...
const addEngine = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Engine added successfully", engine: newEngine });
  } catch (error) {
    console.error(error);
...
const updateEngineStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Engine ${id} updated successfully`, engine });
  } catch (error) {
...
const changeEngine = async (req, res) => {
  ...
  try {
    ...
    const newEngine = await Engine.findOne({ engine_id });
